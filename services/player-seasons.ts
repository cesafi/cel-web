...
export class PlayerSeasonService extends BaseService {
  ...
  static async getBySeasonId(seasonId: number): Promise<ServiceResponse<PlayerSeasonWithDetails[]>> {
    try {
      ...
      const { data, error } = await supabase
        .from(TABLE_NAME)
        .select('*, players(id, ign, first_name, last_name, photo_url, role), schools_teams!inner(id, name, season_id, esport_category_id, seasons(id, name, start_at, end_at), schools(id, name, abbreviation, logo_url), esports_categories(id, division, levels, esports(id, name)))')
  ...
  static async getByPlayerId(playerId: string): Promise<ServiceResponse<PlayerSeasonWithDetails[]>> {
    try {
      ...
      const { data, error } = await supabase
        .from(TABLE_NAME)
        .select('*, players(id, ign, first_name, last_name, photo_url, role), schools_teams(id, name, season_id, esport_category_id, seasons(id, name, start_at, end_at), schools(id, name, abbreviation, logo_url), esports_categories(id, division, levels, esports(id, name)))')
