...
export async function GET(
    ...
    try {
        ...
        const coinTossWinnerAbbr = coinTossWinnerId
            ? (teams.find(t => t.id === coinTossWinnerId)?.abbr || N)
            : N;
        ...
        for (let i = 0; i < maxSlots; i++) {
            const v = vetoList[i];
            if (v) {
                const school = Array.isArray(v.schools_teams?.school)
                    ? v.schools_teams.school[0]
                    : v.schools_teams?.school;
        ...
        // Row 12: ,,,,PICK RATE,pickRate1,pickRate2,...
        csv += row(['', '', '', '', 'PICK RATE', ...pickRates, '', '']);

        // Row 13: ,,,,BAN RATE,banRate1,banRate2,...
        csv += row(['', '', '', '', 'BAN RATE', ...banRates, '', '']);

        // Row 14: ,,,,{BLUE} WR%,wr1,wr2,...
        csv += row(['', '', '', '', `${blueAbbr} WR%`, ...blueWrs, '', '']);

        // Row 15: ,,,,{RED} WR%,wr1,wr2,...
