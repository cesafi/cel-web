...
export async function GET(
    ...
    try {
        ...
        const [draftRes, rosterRes, statsRes] = await Promise.all([
            ...
            isValorant
                ? StatsValorantService.getByGameId(gameId)
                : StatsMlbbService.getByGameId(gameId),
        ]);
        ...
        const teams = parts.map((p: any) => {
            ...
            return { id: p.team.id, name: p.team.name, abbr: school?.abbreviation || N, schoolName: school?.name || N, score: p.match_score || 0, participantId: p.id };
        }).filter(Boolean) as { id: string; name: string; abbr: string; schoolName: string; score: number; participantId: number }[];
        ...
        const bluePicks = draftActions.filter((a: any) => a.team_id === blueId && a.action_type === 'pick').sort((a: any, b: any) => a.sort_order - b.sort_order);
        const redPicks = draftActions.filter((a: any) => a.team_id === redId && a.action_type === 'pick').sort((a: any, b: any) => a.sort_order - b.sort_order);
        ...
        if (isValorant) {
            ...
            // Row 25: MVP header
            csv += row(['MVP', 'MVP IGN', 'SCHOOL FULL NAME', 'SCHOOL ABBREVIATION', 'ROLE', 'PICK', 'KILLS', 'DEATHS', 'ASSISTS', 'ACS', 'ECON RATINGG', 'FIRST BLOODS', 'PLANTS', 'DEFUSES', '']);

            // Row 26: MVP data
            csv += row([
                '', mvpIgn, mvpSchoolName, mvpSchoolAbbr, mvpRole, mvpPick,
                v(mvp?.kills), v(mvp?.deaths), v(mvp?.assists),
                v(mvp?.acs), v(mvp?.econ_rating), v(mvp?.first_bloods),
                v(mvp?.plants), v(mvp?.defuses), ''
            ]);
            ...
            // Row 19: DAMAGE TAKEN
            csv += row(['', ...bStatRow('damage_taken'), '', 'DAMAGE TAKEN', '', ...rev(rStatRow('damage_taken')), '', '', '']);
            ...
            // Row 25: MVP header
            csv += row(['MVP', 'MVP IGN', 'SCHOOL FULL NAME', 'SCHOOL ABBREVIATION', 'ROLE', 'PICK', 'KILLS', 'DEATHS', 'ASSISTS', 'GOLD', 'RATING', 'DAMAGE', 'TURRET', 'DAMAGE TAKEN', 'TEAMFIGHT', 'TURTLES', 'LORDS']);

            // Row 26: MVP data
            csv += row([
                '', mvpIgn, mvpSchoolName, mvpSchoolAbbr, mvpRole, mvpPick,
                v(mvp?.kills), v(mvp?.deaths), v(mvp?.assists),
                v(mvp?.gold), v(mvp?.rating), v(mvp?.damage_dealt),
                v(mvp?.turret_damage), v(mvp?.damage_taken), v(mvp?.teamfight),
                v(mvp?.turtle_slain), v(mvp?.lord_slain)
            ]);
        }
